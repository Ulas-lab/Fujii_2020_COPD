...
plotGeneSetHeatmap <- function(input=norm_anno,
  ...
  if(cat == "Hallmark"|
     ...
     cat == "Motifs"){
    genes <- clusterProfiler::bitr(genes, fromType="ENTREZID", toType="SYMBOL", OrgDb="org.Hs.eg.db")
    genes <- unlist(genes[2],use.names = FALSE)
  }
...
plotVolcano <-  function(comparison,
  ...
  ggplot(data=data, aes(x=anti, y=-log10(padj), colour=regulation)) +
    ...
    scale_x_continuous(limits = c(-abs(max(data$anti))-1, abs(max(data$anti))+1)) +
    scale_y_continuous() +
    xlab("FoldChange") +
    ylab("-log10(padj)") +
