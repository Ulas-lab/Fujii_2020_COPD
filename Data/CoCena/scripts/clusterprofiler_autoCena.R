...
##DO 
  ...
  if(!is.null(DO_enrich) & DO_enrich@result %>%nrow()>0 )
  {
    DO_enrich@result$symbols=
      lapply(1:nrow(DO_enrich@result), 
             function(x) universe_Entrez %>% 
               drop_na(ENTREZID) %>%
               filter(ENTREZID%in%(stri_split_fixed(DO_enrich@result[x,"geneID"], pattern = "/") %>% unlist())) %>%
               pull(SYMBOL) %>%paste0(collapse = "/"))  %>%
      unlist()
...
#TF prediction
 ...
 if(!is.null(TFs) & length(TFs) >0){
   ...
   TFS_intersect <- intersect(TFs,gene_universe)
    ...
    plotTheThing<-function(){
      ...
      p<- ggplot(TFs_matrix_all_plot.melt.mean,aes(x=gene,y=mean,fill=merged)) +
        
        geom_bar(stat="identity",position = "dodge") +
        labs(fill = paste0(voi, collapse = "-") )
...
#  @thomas some weird pdf code
   ...
   if(length(TFS_intersect) != 0L){
    ...
    TFs_matrix <- normdata[,c("merged",head(TFS_intersect,4))]
     ...
     TFs_matrix.melt <- tidyr::gather_(TFs_matrix, "variable", "value",gather_cols = c(head(TFS_intersect,4)) )
