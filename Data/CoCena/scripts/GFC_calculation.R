GFC_calculation <- function() {
  
  if(intersect(voi,colnames(info_dataset))%>% length()>0){
    
    print(paste("variables:",paste0(intersect(voi,colnames(info_dataset)), collapse = ","),
                "will be used as grouping variables,",
                "if these are different in # from voi then please check if",
                "the variables mentioned in voi are present in the metadata"))
    
    corresp_info$grpvar =purrr::pmap(corresp_info[intersect(voi,colnames(info_dataset))], 
                                     paste, sep="-") %>% unlist()
    
  } else {
    
    print(paste("the first column in the metadata will be used as the grouping variable",
                "since the voi is not present in the metadata"))
